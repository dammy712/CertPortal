...
const generateCertificate = async (order: any): Promise<{
  ...
  try {
    ...
    // Generate thumbprint (SHA-1 of DER)
    ...
    // Simple chain = just the cert itself (dev)
...
export const issueCertificate = async (orderId: string, adminId?: string) => {
  ...
  // Send issued email (fire-and-forget)
...
export const downloadCertificate = async (
  ...
  else if (fileType === 'fullchain') fileKey = certificate.certFileKey; // dev: same file
...
export const adminIssueCertificate = async (adminId: string, orderId: string) => {
  // Allow admin to push any PENDING_VALIDATION order to issuance
